    ...
    <div className="flex flex-col gap-y-3 grow">
      <AccountHealthBar
        ...
        borrowLimitProtectedCents={
          simulatedPool?.userBorrowLimitProtectedCents?.toNumber() ??
          pool.userBorrowLimitProtectedCents?.toNumber()
        }
      />

      <LabeledInlineContent
        label={t('trade.positions.status.healthFactor.label')}
        tooltip={t('trade.positions.status.healthFactor.tooltip')}
      >
        <ValueUpdate
          original={
