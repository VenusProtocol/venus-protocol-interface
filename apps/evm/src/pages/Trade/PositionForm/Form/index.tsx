...
export const Form: React.FC<FormProps> = ({
  ...
  const { data: getSimulatedTradeMutationsData } = useSimulateTradeMutations({
    dsaAmountTokens: new BigNumber(formValues.dsaAmountTokens || 0),
    position,
    balanceMutations,
    action,
  });
  const simulatedPosition = getSimulatedTradeMutationsData?.position;
  ...
  > = {
    ...
    label: t('trade.operationForm.openForm.dsaFieldLabel'),
    disabled: isDisabled,
      ...
      {action === 'open' && (
        <>
          <SelectDsaTokenTextField
            ...
            placeholder={t('trade.operationForm.openForm.dsaFieldPlaceholder')}
            autoFocus
      ...
      {(action === 'open' || action === 'increase' || action === 'reduce') && (
        <>
          <TokenTextField
            ...
            label={t('trade.operationForm.openForm.longFieldLabel')}
            disabled
            description={
              !!accountAddress &&
              !isSubmitting &&
          ...
          <TokenTextField
            ...
            label={t('trade.operationForm.openForm.shortFieldLabel')}
            disabled={isDisabled}
            hasError={
              canDisplayFormError &&
