...
if (!position) {
  throw new Error('Expected Trade mock positions');
}
...
describe('IncreaseForm', () => {
  ...
  const mockUseIncreaseTradePosition = useIncreaseTradePosition as Mock;
  const mockIncreaseTradePosition = vi.fn();
  ...
  const setReadyState = ({
    ...
    mockUseIncreaseTradePosition.mockImplementation(() => ({
      mutateAsync: mockIncreaseTradePosition,
      isPending: false,
    }));
  };
  ...
  it('submits scale position request with transformed values when form is valid', async () => {
    ...
    await waitFor(() => expect(mockIncreaseTradePosition).toHaveBeenCalledTimes(1));

    expect(mockIncreaseTradePosition).toHaveBeenCalledWith({
