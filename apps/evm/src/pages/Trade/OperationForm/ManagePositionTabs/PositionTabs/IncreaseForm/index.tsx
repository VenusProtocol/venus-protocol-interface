...
export const IncreaseForm: React.FC<IncreaseFormProps> = ({ position }) => {
  ...
  const { mutateAsync: increaseTradePosition, isPending: isSubmitting } = useIncreaseTradePosition({
  ...
  const balanceMutations: AssetBalanceMutation[] = [
    {
      ...
      label: t('trade.operationForm.openForm.long'),
    },
    {
      type: 'asset',
      ...
      label: t('trade.operationForm.openForm.short'),
    },
  ];
  ...
  return (
    <PositionForm
      ...
      submitButtonLabel={t('trade.operationForm.increaseForm.submitButtonLabel')}
    />
  );
};
