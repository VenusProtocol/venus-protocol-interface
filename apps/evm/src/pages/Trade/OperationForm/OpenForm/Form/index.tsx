...
export const Form: React.FC<FormProps> = ({ position: newPosition }) => {
  ...
  const { mutateAsync: openTradePosition, isPending: isSubmitting } = useOpenTradePosition({
  ...
  return (
    <PositionForm
      ...
      submitButtonLabel={t('trade.operationForm.openForm.submitButtonLabel')}
    />
  );
};
