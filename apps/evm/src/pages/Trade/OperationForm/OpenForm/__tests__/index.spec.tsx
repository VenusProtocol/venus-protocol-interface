...
describe('OpenForm', () => {
  ...
  const mockUseOpenTradePosition = useOpenTradePosition as Mock;
  const mockOpenTradePosition = vi.fn();
  ...
  const setReadyState = ({
    ...
    mockUseOpenTradePosition.mockImplementation(() => ({
      mutateAsync: mockOpenTradePosition,
      isPending: false,
    }));
  };
  ...
  it('submits open position request with transformed values when form is valid', async () => {
    ...
    await waitFor(() => expect(mockOpenTradePosition).toHaveBeenCalledTimes(1));

    expect(mockOpenTradePosition).toHaveBeenCalledWith({
