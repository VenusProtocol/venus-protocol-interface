...
import { useGetNewTradePosition } from '..';
...
const setHookState = (
  input: {
    ...
    shortToken?: Token;
    dsaAssets?: Asset[];
    isGetTradeAssetsLoading?: boolean;
    getPoolData?:
      | {
          pool: typeof pool;
        }
  ...
  (useGetTradeAssets as Mock).mockImplementation(() => ({
    ...
    isLoading: isGetTradeAssetsLoading,
  }));
...
describe('useGetNewTradePosition', () => {
  beforeEach(() => {
    setHookState();
  });

  it('passes the connected wallet address and core pool address to downstream hooks', () => {
    renderHook(() => useGetNewTradePosition(), {
      accountAddress: fakeAddress,
    });

    expect(useGetTradeAssets).toHaveBeenCalledWith({
      accountAddress: fakeAddress,
    });
  ...
  it('returns a loading state while the core pool query is loading', () => {
    ...
    const { result } = renderHook(() => useGetNewTradePosition());
  ...
  it('returns a loading state while Trade assets are loading', () => {
    setHookState({
      isGetTradeAssetsLoading: true,
    });
