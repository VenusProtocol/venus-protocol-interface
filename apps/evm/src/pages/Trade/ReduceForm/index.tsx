...
export const ReduceForm: React.FC<ReduceFormProps> = ({ position, closePosition = false }) => {
  ...
  const { data, isLoading, error } = useGetTradeReduceSwapQuotes(
    {
  ...
  return (
    <PositionForm
      ...
      submitButtonLabel={
        closePosition
          ? t('trade.operationForm.reduceForm.submitButtonLabel.close')
          : t('trade.operationForm.reduceForm.submitButtonLabel.reduce')
      }
    />
  );
};
