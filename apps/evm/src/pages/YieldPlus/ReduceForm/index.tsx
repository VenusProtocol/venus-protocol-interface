...
export const ReduceForm: React.FC<ReduceFormProps> = ({ position, closePosition = false }) => {
  ...
  const closeRatio = useMemo(
    () =>
      position.shortBalanceTokens.isZero()
        ? new BigNumber(0)
        : debouncedShortAmountTokens.dividedBy(position.shortBalanceTokens),
    [debouncedShortAmountTokens, position.shortBalanceTokens],
  );
  ...
  const {
    ...
    },
    {
      enabled:
        ...
        !!longProfitAmountDeltaTokens?.isGreaterThan(0) &&
        isProfitable,
    },
  );
    ...
    if (debouncedShortAmountTokens) {
      setFormValues(currFormValues => {
        ...
        if (sanitizedShortAmountTokens.isGreaterThan(0)) {
          const newLongAmountToWithdrawTokens = position.longBalanceTokens.multipliedBy(closeRatio);
  ...
  const handleSubmit = async () => {
    ...
    const closeFractionPercentage = closeRatio.multipliedBy(100).toNumber();

    const isReducingWithProfit =
      pnlDsaTokens?.isPositive() &&
      repayWithProfitSwapQuote?.direction === 'approximate-out' &&
      profitSwapQuote?.direction === 'exact-in';

    // Reduce with profit
    if (isReducingWithProfit && !closePosition) {
    ...
    const isReducingWithLoss =
      pnlDsaTokens?.isLessThanOrEqualTo(0) &&
      repayWithLossSwapQuote?.direction === 'exact-in' &&
