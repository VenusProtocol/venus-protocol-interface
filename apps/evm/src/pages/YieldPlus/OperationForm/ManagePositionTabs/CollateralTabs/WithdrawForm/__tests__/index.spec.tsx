...
const mockUseWithdrawYieldPlusPositionCollateral = useWithdrawYieldPlusPositionCollateral as Mock;
const mockWithdrawYieldPlusPositionCollateral = vi.fn();
...
describe('WithdrawForm', () => {
  const setReadyState = ({ simulatedPool = position.pool }: { simulatedPool?: Pool } = {}) => {
    ...
    mockUseWithdrawYieldPlusPositionCollateral.mockImplementation(() => ({
      mutateAsync: mockWithdrawYieldPlusPositionCollateral,
      isPending: false,
    }));
  };
  ...
  it('renders form when data is ready', async () => {
    ...
    await waitFor(() =>
      expect(
        getByText(en.yieldPlus.operationForm.withdrawDsaForm.submitButtonLabel),
      ).toBeInTheDocument(),
    );
  ...
  it('submits withdraw request when form is valid', async () => {
    ...
    await waitFor(() => expect(mockWithdrawYieldPlusPositionCollateral).toHaveBeenCalledTimes(1));

    expect(mockWithdrawYieldPlusPositionCollateral).toHaveBeenCalledWith({
