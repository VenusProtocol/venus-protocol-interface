...
describe('IncreaseForm', () => {
  ...
  const mockUseScaleYieldPlusPosition = useScaleYieldPlusPosition as Mock;
  const mockScaleYieldPlusPosition = vi.fn();
  ...
  const setReadyState = ({
    ...
    mockUseScaleYieldPlusPosition.mockImplementation(() => ({
      mutateAsync: mockScaleYieldPlusPosition,
      isPending: false,
    }));
  };
  ...
  it('submits scale position request with transformed values when form is valid', async () => {
    ...
    await waitFor(() => expect(mockScaleYieldPlusPosition).toHaveBeenCalledTimes(1));

    expect(mockScaleYieldPlusPosition).toHaveBeenCalledWith({
