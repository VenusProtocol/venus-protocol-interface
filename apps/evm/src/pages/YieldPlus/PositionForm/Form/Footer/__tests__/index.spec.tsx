...
const entryPriceLabel = t('yieldPlus.operationForm.openForm.entryPrice');
const averageEntryPriceLabel = t('yieldPlus.operationForm.openForm.averageEntryPrice');
const pnlLabel = t('yieldPlus.operationForm.openForm.pnl');
...
describe('YieldPlus PositionForm Footer', () => {
  ...
  it('renders liquidation price, entry price and net APY rows for open action', () => {
    const simulatedPosition = {
      ...
      priceTokens: new BigNumber('1.23'),
  ...
  it('renders original and updated values for increase action', () => {
    const simulatedPosition = {
      ...basePosition,
      liquidationPriceTokens: new BigNumber('145.67'),
      averageEntryPriceTokens: new BigNumber('0.98'),
      netApyPercentage: 4.56,
    };
    ...
    const averageEntryPriceRow =
      screen.getByText(averageEntryPriceLabel).parentElement?.parentElement;
    const netApyRow = screen.getByText(netApyLabel).parentElement?.parentElement;

    expect(liquidationPriceRow?.textContent).toMatchInlineSnapshot(`"Liq. priceN/A145.67"`);
    expect(averageEntryPriceRow?.textContent).toMatchInlineSnapshot(`"Avg. entry price10.98"`);
    expect(netApyRow?.textContent).toMatchInlineSnapshot(`"Net APY0%4.56%"`);
  });
  ...
  it('does not render position rows or account health for close action', () => {
    ...
    expect(screen.queryByText(averageEntryPriceLabel)).not.toBeInTheDocument();
