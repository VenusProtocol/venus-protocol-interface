...
export type CloseYieldPlusPositionInput = {
  longVTokenAddress: Address;
  shortVTokenAddress: Address;
};

type Options = UseSendTransactionOptions<CloseYieldPlusPositionInput>;

export const useCloseYieldPlusPosition = (options?: Partial<Options>) => {
  const { address: relativePositionManagerContractAddress } = useGetContractAddress({
    name: 'RelativePositionManager',
  });

  return useSendTransaction({
    fn: ({ longVTokenAddress, shortVTokenAddress }: CloseYieldPlusPositionInput) => {
