...
import {
  type CloseYieldPlusPositionWithProfitInput,
  useCloseYieldPlusPositionWithProfit,
} from '..';

vi.mock('libs/contracts');

const fakeInput: CloseYieldPlusPositionWithProfitInput = {
...
describe('useCloseYieldPlusPositionWithProfit', () => {
  it('calls useSendTransaction with correct parameters', async () => {
    renderHook(() => useCloseYieldPlusPositionWithProfit(fakeOptions));
  ...
  it('defaults repay swap values when repaySwapQuote is not provided', async () => {
    renderHook(() => useCloseYieldPlusPositionWithProfit());
