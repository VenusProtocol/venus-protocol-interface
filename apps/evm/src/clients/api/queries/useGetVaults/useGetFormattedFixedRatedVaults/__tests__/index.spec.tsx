...
const fakeMatrixdockVaultProduct: GetFixedRatedVaultsOutput[number] = {
...
const mockInstitutionalCollateralToken: Token = {
  chainId: 97,
...
const pendleVaultAsset: Asset = {
  ...
  userSupplyBalanceProtectedCents: new BigNumber('326742'),
...
const fakeVaultProducts = [fakePendleVaultProduct, fakeMatrixdockVaultProduct];
...
describe('useGetFormattedFixedRatedVaults', () => {
  beforeEach(() => {
    ...
    (useGetTokens as Mock).mockReturnValue([
      ...
      bnbToken,
      mockUsdcToken,
      mockInstitutionalCollateralToken,
    ]);
    ...
    (useGetInstitutionalVaultUserData as Mock).mockReturnValue({
      data: [
        {
          vaultAddress: fakeMatrixdockVaultProduct.vaultAddress,
  ...
  it('formats matrixdock vault user metrics and status', () => {
    ...
    expect(institutionalVault).toBeDefined();
    expect(institutionalVault?.openStartDate).toEqual(new Date('2026-04-07T00:00:00.000Z'));
    expect(institutionalVault?.status).toBe(VaultStatus.Pending);
    expect(institutionalVault?.userStakeBalanceMantissa?.toFixed()).toBe('100000000');
    expect(institutionalVault?.userRedeemLimitMantissa.toFixed()).toBe('25000000');
    expect(institutionalVault?.userWithdrawLimitMantissa.toFixed()).toBe('50000000');
    expect(institutionalVault?.userYieldTokens).toBeUndefined();
  });

  it('derives userYieldTokens from on-chain withdrawable assets for liquidated vaults', () => {
    (useGetFixedRatedVaults as Mock).mockReturnValue({
      data: [
        {
