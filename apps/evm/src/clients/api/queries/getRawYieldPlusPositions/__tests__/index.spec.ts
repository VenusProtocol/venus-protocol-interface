...
describe('getRawYieldPlusPositions', () => {
  ...
  it('defaults nullable API fields to zero', async () => {
    ...
    expect(position.averageEntryPriceTokens.isZero()).toBe(true);
    expect(position.totalShortOpenedTokens.isZero()).toBe(true);
    expect(position.totalLongReceivedTokens.isZero()).toBe(true);
