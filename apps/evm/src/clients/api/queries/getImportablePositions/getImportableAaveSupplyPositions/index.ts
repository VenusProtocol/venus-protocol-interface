...
export const getImportableAaveSupplyPositions = async ({
  ...
  userReservesData.result.forEach(data => {
    ...
    data.forEach(({ underlyingAsset, scaledATokenBalance, scaledVariableDebt }) => {
      ...
      userABalanceMapping[underlyingAsset.toLowerCase() as Address] = {
  ...
  reservesData.result.forEach(data => {
    ...
    data.forEach(({ liquidityRate, liquidityIndex, underlyingAsset, aTokenAddress }) => {
      const tokenAddress = underlyingAsset.toLowerCase() as Address;

      // Skip assets for which user has no supply position
      if (!Object.prototype.hasOwnProperty.call(userABalanceMapping, tokenAddress)) {
        return;
      }

      const userABalances = userABalanceMapping[tokenAddress];
      ...
      const userSupplyBalanceMantissa = BigInt(
        Math.floor(
          // We use numbers here because the exchange rate uses decimals
          Number(userABalances.supplyBalanceMantissa) * (Number(liquidityIndex) / Number(RAY)),
        ),
      );
