...
describe('getFixedRatedVaultUserStakedTokens', () => {
  it('returns undefined tokensMantissa for all vaults when accountAddress is falsy', async () => {
    ...
    expect(result).toEqual([
      { vaultAddress: fakeVaultAddresses[0], tokensMantissa: undefined },
      { vaultAddress: fakeVaultAddresses[1], tokensMantissa: undefined },
    ]);
  });
  ...
  it('returns undefined tokensMantissa for failed multicall results', async () => {
    const mockPublicClient = {
      multicall: vi.fn().mockResolvedValue([
        { status: 'success', result: 500000000000000000n },
