...
export const InstitutionalVaultCard: React.FC<InstitutionalVaultCardProps> = ({
  ...
  const hasMatured = now.getTime() > vault.maturityDate.getTime();
  ...
  const formattedOpenEndDate = t('vault.card.textualWithTime', {
    date: vault.openEndDate,
  });

  const clickAble = [VaultStatus.Active, VaultStatus.Earning, VaultStatus.Deposit].includes(
    vault.status,
  );

  const footerLabel = hasMatured ? t('vault.card.claimReward') : t('vault.card.youStaked');
        ...
        <div className={cn('flex flex-col gap-y-4')}>
          ...
          >
            <div className="flex items-center gap-x-1">
              <Icon name="star" className="h-4 w-4 text-green" />
          ...
          >
            <div className="text-b1r text-end">
              ...
              <div className="flex items-center justify-end gap-x-3 mt-2">
                <div className="w-[100px] h-2 rounded-full bg-[rgb(45,53,73)] overflow-hidden">
                  <div
