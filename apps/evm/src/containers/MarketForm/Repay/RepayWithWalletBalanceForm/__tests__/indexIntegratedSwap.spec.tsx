...
describe('RepayWithWalletBalanceForm - Feature flag enabled: integratedSwap', () => {
  ...
  it('disables submit button if no swap is found', async () => {
    (useGetSwapQuote as Mock).mockImplementation(() => ({
      ...
      error: {
        ...
        message: en.marketForm.error.noSwapQuoteFound,
      },
      isLoading: false,
    }));
  ...
  it('disables submit button if amount entered in input is higher than wallet balance', async () => {
    ...
    await waitFor(() =>
      expect(
        getByText(
          en.marketForm.error.higherThanWalletBalance.replace('{{tokenSymbol}}', busd.symbol),
        ),
      ).toBeInTheDocument(),
    );
  ...
  it('disables submit button if amount entered in input would have a higher value than repay balance after swapping', async () => {
    ...
    // Check error is displayed
    await waitFor(() =>
      expect(getByText(en.marketForm.error.higherThanRepayBalance)).toBeInTheDocument(),
    );
  ...
  it('displays acknowledgement toggle and enables submit button after acknowledgement when swap has high price impact', async () => {
    ...
    await waitFor(() =>
      expect(getByText(en.marketForm.acknowledgements.highPriceImpact.label)).toBeInTheDocument(),
    );
    ...
    await checkSubmitButtonIsEnabled({
      textContent: en.marketForm.submitButtonLabel.repay,
    });
  });
  ...
  it('disables submit button when price impact has reached the maximum tolerated', async () => {
    ...
    // Check error is displayed
    await waitFor(() =>
      expect(getByText(en.marketForm.error.priceImpactTooHigh)).toBeInTheDocument(),
    );
  ...
  it('updates input value to 0 when clicking on MAX button if wallet balance is 0', async () => {
    ...
    // Click on MAX button
    fireEvent.click(getByText(en.marketForm.rightMaxButtonLabel));
  ...
  it('lets user swap and repay partial loan then calls onClose callback on success', async () => {
    ...
    await waitFor(() => getByText(en.marketForm.submitButtonLabel.repay));
    fireEvent.click(getByText(en.marketForm.submitButtonLabel.repay));
  ...
  it('lets user swap and repay full loan', async () => {
    ...
    await waitFor(() => expect(getByText(en.marketForm.repay.fullRepaymentWarning)).toBeTruthy());
