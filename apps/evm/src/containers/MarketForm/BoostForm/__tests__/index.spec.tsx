...
describe('BoostForm', () => {
  ...
  it('disables form and displays a warning notice if the borrow cap of this market has been reached', async () => {
    ...
    await waitFor(() =>
      expect(
        getByText(en.marketForm.error.borrowCapReached.replace('{{assetBorrowCap}}', '100 XVS')),
      ).toBeInTheDocument(),
    );
  ...
  it('disables form and displays a warning notice if user has not supplied and collateralized any tokens yet', async () => {
    ...
    await waitFor(() =>
      expect(
        getByText(
          en.marketForm.error.noCollateral.replace(
            '{{tokenSymbol}}',
  ...
  it('disables submit button and displays a warning notice if borrow amount entered is higher than asset liquidity', async () => {
    ...
    // Check warning is displayed
    await waitFor(() =>
      expect(getByText(en.marketForm.error.higherThanAvailableLiquidity)).toBeInTheDocument(),
    );
  ...
  it('disables submit button and displays a warning notice if borrow amount entered is higher than asset borrow cap', async () => {
    ...
    await waitFor(() =>
      expect(
        getByText(
          en.marketForm.error.higherThanBorrowCap
  ...
  it('disables submit button if position would liquidate user', async () => {
    ...
    // Check warning is displayed
    await waitFor(() => expect(getByText(en.marketForm.error.tooRisky)).toBeInTheDocument());
  ...
  it('prompts user to acknowledge risk if position would lower health factor to risky threshold', async () => {
    ...
    // Check warning is displayed
    expect(getByText(en.marketForm.acknowledgements.riskyOperation.tooltip));

    // Check submit button is disabled
    const submitButton = document.querySelector('button[type="submit"]') as HTMLButtonElement;
    await waitFor(() =>
      expect(submitButton).toHaveTextContent(en.marketForm.submitButtonLabel.boost),
    );
  ...
  it('lets user open a leveraged position using a swap', async () => {
    ...
    await waitFor(() => getByText(en.marketForm.submitButtonLabel.boost));
    fireEvent.click(getByText(en.marketForm.submitButtonLabel.boost));
