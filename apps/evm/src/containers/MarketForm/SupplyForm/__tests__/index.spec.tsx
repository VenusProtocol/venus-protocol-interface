...
describe('SupplyForm', () => {
  ...
  it('disables form and displays a warning notice if the supply cap of this market has been reached', async () => {
    ...
    await waitFor(() =>
      expect(
        getByText(en.marketForm.error.supplyCapReached.replace('{{assetSupplyCap}}', '100 XVS')),
      ).toBeInTheDocument(),
    );
  ...
  it('lets user supply BNB', async () => {
    ...
    await waitFor(() =>
      expect(submitButton).toHaveTextContent(en.marketForm.submitButtonLabel.supply),
    );
