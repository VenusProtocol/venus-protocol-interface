...
describe('useSimulateYieldPlusMutations', () => {
  ...
  it('recalculates the average entry price when the simulated position adds long and short exposure', () => {
    ...
    expect(
      result.current.data?.position.averageEntryPriceTokens.isEqualTo(new BigNumber(0.5)),
    ).toBe(true);
  });

  it('keeps the existing average entry price when no long tokens are added', () => {
    (useGetSimulatedPool as Mock).mockReturnValue({
      data: {
        pool: makeSimulatedPool({
          shortBalanceTokens: position.shortBalanceTokens.plus(10),
    ...
    expect(
      result.current.data?.position.averageEntryPriceTokens.isEqualTo(
        position.averageEntryPriceTokens,
      ),
    ).toBe(true);
  });
});
