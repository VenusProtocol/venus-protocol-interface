...
const generatePancakeSwapTokens = async () => {
  ...
  const pancakeSwapTokenList = (await data.json()) as {
    tokens: PancakeSwapToken[];
  };
...
`;
  ...
  // Generate file
  const outputFilePath = path.resolve('./src/constants/tokens/swap/mainnetPancakeSwapTokens.ts');
