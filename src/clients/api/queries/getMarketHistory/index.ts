...
const getMarketHistory = async ({
  ...
  const response = await restService<GetMarketHistoryResponse, 'v2'>({
    endpoint,
    method: 'GET',
    next: true,
  });
  ...
  let sampleSize = response.data?.result.length || 0;
  ...
  const marketSnapshots = response.data?.result?.slice(-sampleSize) || [];
