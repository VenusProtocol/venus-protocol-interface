...
describe('hooks/useSupplyWithdrawModal/Supply', () => {
  ...
  it('submit is disabled with no amount', async () => {
    ...
    const disabledButtonText = getByText(
      en.supplyWithdrawModal.supply.submitButtonLabel.enterValidAmount,
    );
  ...
  it('disables form and displays a warning notice if the supply cap of this market has been reached', async () => {
    ...
    // Check submit button is disabled
    expect(
      getByText(en.supplyWithdrawModal.supply.submitButtonLabel.supplyCapReached).closest('button'),
    ).toBeDisabled();
  ...
  it('disables submit button and displays error notice if an amount entered in input is higher than asset supply cap', async () => {
    ...
    await waitFor(() =>
      getByText(en.supplyWithdrawModal.supply.submitButtonLabel.amountHigherThanSupplyCap),
    );
    expect(
      getByText(en.supplyWithdrawModal.supply.submitButtonLabel.amountHigherThanSupplyCap).closest(
        'button',
      ),
    ).toBeDisabled();
  });
  ...
  it('lets user supply BNB, then displays successful transaction modal and calls onClose callback on success', async () => {
    ...
    await waitFor(() =>
      expect(submitButton).toHaveTextContent(
        en.supplyWithdrawModal.supply.submitButtonLabel.supply,
      ),
    );
    ...
    await waitFor(() =>
      expect(openSuccessfulTransactionModal).toHaveBeenCalledWith({
        ...
        content: en.supplyWithdrawModal.supply.successfulTransactionModal.message,
        title: en.supplyWithdrawModal.supply.successfulTransactionModal.title,
      }),
    );
  });
